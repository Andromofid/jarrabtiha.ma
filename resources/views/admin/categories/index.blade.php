...
@section('content')
    ...
    <section class="mb-8 rounded-card border border-border bg-white p-6 shadow-soft sm:p-8">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h2 class="mt-2 font-display text-4xl font-bold text-ink">Shape the catalog structure</h2>
    ...
    @if ($categories->isEmpty())
        <section class="rounded-[2rem] border border-dashed border-border bg-white px-8 py-16 text-center shadow-soft">
            <h3 class="font-display text-3xl font-bold text-ink">No categories yet</h3>
                    ...
                    <div class="flex flex-col gap-5 p-5 sm:p-6">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <h3 class="mt-1 text-xl font-semibold text-ink">{{ $category->name }}</h3>
                                <p class="mt-2 truncate text-sm text-ink-light">{{ $category->slug }}</p>
                            </div>
                        ...
                        <div class="grid gap-3 sm:grid-cols-3">
                            ...
                            <div class="rounded-2xl bg-cream px-4 py-3">
                                <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Children</p>
                                <p class="mt-2 text-sm font-semibold text-ink">{{ $category->children_count }}</p>
                            </div>
                            <div class="rounded-2xl bg-cream px-4 py-3">
                                <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Products</p>
                                <p class="mt-2 text-sm font-semibold text-ink">{{ $category->products_count }}</p>
                            </div>
                        </div>
