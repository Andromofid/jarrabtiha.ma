    ...
    <div class="grid gap-8 lg:grid-cols-[minmax(0,1.55fr)_minmax(19rem,0.95fr)]">
                ...
                <div class="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                    <div class="flex gap-4">
                        ...
                        <div class="min-w-0">
                            ...
                            <h2 class="mt-2 text-3xl font-semibold text-ink sm:text-4xl">{{ $user->name }}</h2>
                            <p class="mt-2 break-all text-sm text-ink-soft">{{ $user->email }}</p>
            ...
            <section class="rounded-card border border-border bg-white p-6 shadow-soft sm:p-8">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <h3 class="mt-2 font-display text-3xl font-bold text-ink">Recent contributions</h3>
                    </div>
                </div>

                @if ($reviews->isEmpty())
                    <div class="mt-6 rounded-[1.75rem] border border-dashed border-border bg-cream px-6 py-12 text-center text-ink-soft">
                            ...
                            <article class="rounded-[1.75rem] border border-border bg-cream p-5">
                                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        <span class="{{ $review->is_approved ? 'bg-success-soft' : 'bg-danger-soft' }} rounded-full px-3 py-1 text-xs font-semibold text-ink">
                                ...
                                <div class="mt-4 grid gap-3 sm:grid-cols-4">
                                    <div class="rounded-2xl border border-border bg-white px-4 py-3">
                                        <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Likes</p>
                                        <p class="mt-2 text-sm font-semibold text-ink">{{ $review->likes_count }}</p>
                                    </div>
                                    <div class="rounded-2xl border border-border bg-white px-4 py-3">
                                        <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Recommend</p>
                                        <p class="mt-2 text-sm font-semibold text-ink">{{ $review->would_recommend ? 'Yes' : 'No' }}</p>
                                    </div>
                                    <div class="rounded-2xl border border-border bg-white px-4 py-3">
                                        <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Skin type</p>
                                        <p class="mt-2 text-sm font-semibold capitalize text-ink">{{ $review->skin_type ?: 'Not set' }}</p>
                                    </div>
                                    <div class="rounded-2xl border border-border bg-white px-4 py-3">
                                        <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Hair type</p>
                                        <p class="mt-2 text-sm font-semibold capitalize text-ink">{{ $review->hair_type ?: 'Not set' }}</p>
                                    </div>
                                </div>

                                <div class="mt-4 flex flex-wrap gap-4 text-xs font-medium text-ink-light">
        ...
        <aside class="space-y-6">
            <section class="rounded-card border border-border bg-white p-5 shadow-soft sm:p-6">
                <h3 class="text-xl font-semibold text-ink">Account details</h3>
                <div class="mt-5 space-y-4">
                    ...
                    <div class="rounded-2xl bg-cream px-4 py-3">
                        <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Joined</p>
                        <p class="mt-2 text-sm font-semibold text-ink">{{ $user->created_at->format('M d, Y') }}</p>
                    </div>
                </div>
            </section>

            <section class="rounded-card border border-border bg-white p-5 shadow-soft sm:p-6">
                <h3 class="text-xl font-semibold text-ink">Engagement</h3>
                <div class="mt-5 space-y-4">
                    <div class="rounded-2xl bg-cream px-4 py-3">
                    ...
                    <div class="rounded-2xl bg-cream px-4 py-3">
                        <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Recommended reviews</p>
                        <p class="mt-2 text-sm font-semibold text-ink">{{ $recommendedCount }}</p>
                    </div>
                    <div class="rounded-2xl bg-cream px-4 py-3">
                        <p class="text-xs uppercase tracking-[0.18em] text-ink-light">Verification date</p>
