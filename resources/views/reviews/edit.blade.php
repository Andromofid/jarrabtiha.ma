    ...
    <main class="mx-auto max-w-4xl px-6 py-12">
        <section class="relative overflow-hidden rounded-card border border-border bg-white p-8 shadow-soft sm:p-10">
            ...
            <div class="relative">
                ...
                <p class="mt-6 text-sm font-semibold uppercase tracking-[0.18em] text-ink-light">
                    Modifier mon avis
                </p>
            ...
            <form method="POST" action="{{ route('reviews.update', $review) }}" class="space-y-6">
                ...
                <div x-data="{ rating: {{ old('rating', $review->rating) }} }" class="space-y-6">
                    ...
                    <div class="rounded-2xl border border-border bg-cream p-6 text-center">
                        ...
                        @error('rating')
                        <p class="mt-4 text-sm font-medium text-danger">{{ $message }}</p>
                        @enderror
                    </div>

                    <div>
                        <label for="body" class="mb-2 block text-sm font-semibold text-ink">
                            Ton exp̩rience
                        </label>
                        <textarea
                            id="body"
                            ...
                            rows="5"
                            required
                            class="w-full rounded-2xl border border-border bg-cream px-4 py-3 text-sm text-ink placeholder:text-ink-light focus:border-primary focus:ring-0 focus:outline-none resize-none transition"
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label for="result_duration" class="mb-2 block text-sm font-semibold text-ink">
                            ...
                            <select
                                ...
                                required
                                class="w-full rounded-xl border border-border bg-cream px-4 py-3 text-sm text-ink focus:border-primary focus:ring-0 focus:outline-none transition">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row">
                        ...
                        <a href="{{ route('products.show', $review->product->slug) }}"
                            class="inline-flex items-center justify-center rounded-pill border border-border bg-white px-6 py-3 text-sm font-semibold text-ink transition hover:border-primary/30 hover:text-primary">
                            Voir le produit
                        </a>
                    </div>
                </div>
